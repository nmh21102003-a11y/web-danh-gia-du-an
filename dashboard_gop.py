...
def plot_stacked_chart(df_long, list_cows, col_tc):
    ...
    return alt.Chart(df_chart).mark_bar(size=40).encode(
        ...
        color=alt.Color(f'{col_tc}:N', scale=alt.Scale(range=custom_colors), legend=alt.Legend(title="Tiêu chí", orient='bottom', labelLimit=800)),
...
try:
    ...
    with tab3:
        ...
        st.info("💡 **Cách đọc:** Cột xanh là đóng góp (TC 1&2). Đường đỏ là tiêu chí cảnh báo (TC 3&4).")
